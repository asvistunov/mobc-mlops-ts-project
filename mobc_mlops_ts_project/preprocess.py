...
@hydra.main(
    version_base=None, config_path=str(CONF_PATH), config_name="preprocess"
)
def main(cfg: DictConfig):
    sales = pd.read_csv(DATA_PATH / "raw/sales_train_evaluation.csv").query(
        f'state_id == "{cfg.filters.state_id}"'
    )
    ...
    sales_clean = sales_long.merge(
        calendar[["date", "d"]], on="d", validate="many_to_one"
    )[
        [
            "date",
            "item_id",
    ...
    sales_clean_filtered = sales_clean[
        sales_clean.dept_id == str(cfg.filters.dept_id)
    ]

    (
        sales_clean_filtered.query(
            f'dept_id == "{cfg.filters.dept_id}"'
        ).to_csv(DATA_PATH / "processed/sales_clean.csv", index=False)
    )

    sales_clean_filtered.set_index(
        OmegaConf.to_object(cfg.filters.group_cols), inplace=True
    )
    ...
    test_ids = np.random.choice(
        ids, int(len(ids) * cfg.train_test_split.test_size), replace=False
    )
    ...
    for data, split_type in zip([train, test], ["train", "test"]):
        (
            data.reset_index().to_csv(
                DATA_PATH / f"processed/{split_type}.csv", index=False
            )
        )
